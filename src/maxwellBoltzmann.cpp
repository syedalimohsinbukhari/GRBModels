...
vd MaxwellBoltzmannDistributionPDF(const vd &xArray,
    ...
    vd output = allocateVector(xArray);
    const vd unNorm = unNormalizedMaxwellBoltzmannDistribution(xArray, a);
...
vd MaxwellBoltzmannDistributionCDF(const vd &xArray,
    ...
    FOR_LOOP(xArray, {
        const double_t erfValue = erf(xArray[i] / (sqrt(2) * a));
        const double_t amplitudeFactor = sqrt(2 / M_PI) * (xArray[i] / a);
        const double_t exponentialFactor = -(xArray[i] * xArray[i]) / denominator_factor;
